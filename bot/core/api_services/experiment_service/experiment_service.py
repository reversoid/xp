...
from modules.experiment.services.exceptions import (
...
from shared.utils.convert.message_to_payload import combine_upload_info_requests
...
class ExperimentService(ApiService):
    ...
    async def get_user_current_experiment(self, tg_user_id: int) -> Experiment | None:
        ...
        experiment: Experiment | None = await self.get(
            url, headers=headers, dataclass=Experiment
        )
        return experiment
