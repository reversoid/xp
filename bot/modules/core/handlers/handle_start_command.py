...
async def handle_start_command(message: Message, state: FSMContext):
    ...
    if response.registered:
        ...
        await message.answer_video(
            video="CgACAgIAAxkBAAIIkWaC-mZtOFXVSZERgxKJasvkZqE4AAKoUwACyAURSFIoxT4AASK2gTUE",
            caption=f'Привет, {response.username} \n\n{CORE_LEXICON["cmd_start"]}',
        ...
        await message.answer(
            text=CORE_LEXICON["cmd_start"], reply_markup=ReplyKeyboardRemove()
        )
        await message.answer(text=CORE_LEXICON["fill_username"])


@start_router.message(
    StateFilter(FSMRegistration.fill_username), F.text.regexp(username_pattern)
)
