...
@experiments_router.message(
    StateFilter(FSMProfile.viewing_experiments),
    F.text == BUTTON_LEXICON["load_more_experiments"],
)
async def show_experiments(
    ...
    lower_bound = (
        ...
        and message.text == BUTTON_LEXICON["load_more_experiments"]
        else None
    )
    ...
    (
        await bot.send_message(chat_id=user_id, text=LEXICON["your_experiments"])
        if not lower_bound
        else None
    )
    ...
    if not experiments.next_key:
        ...
        await bot.send_message(
            ...
            reply_markup=next_experiments_keyboard,
        )


@experiments_router.message(
    StateFilter(FSMProfile.viewing_experiments),
    F.text == BUTTON_LEXICON["cancel_showing_experiments"],
)
