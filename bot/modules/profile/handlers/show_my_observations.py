...
async def show_observations(message: Message | CallbackQuery, state: FSMContext, bot: Bot, query: CallbackQuery | None = None):
    data = await state.get_data()
    lower_bound = data.get(
        'lower_bound', None) if isinstance(message, Message) and message.text == BUTTON_LEXICON['load_more_observations'] else None
    ...
    await bot.send_message(chat_id=user_id, text=LEXICON['your_observations']) if not lower_bound else None

    await send_observations(bot=bot, tg_user_id=user_id, observations=observations.items)

    if not observations.next_key:
        await state.clear()
        await bot.send_message(chat_id=user_id, text=LEXICON['no_more_observations'], reply_markup=ReplyKeyboardRemove())
        return
    else:
        await state.set_state(FSMProfile.viewing_observations)
        await state.update_data(lower_bound=observations.next_key)
        await bot.send_message(chat_id=user_id, text=LEXICON['exists_more_observations'], reply_markup=next_observations_keyboard)


@observations_router.message(StateFilter(FSMProfile.viewing_observations), F.text == BUTTON_LEXICON['cancel_showing_observations'])
